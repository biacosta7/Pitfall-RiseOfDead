...
void InitZombieHands(ZombieHand hands[], int count, Platform platforms[], int total_ground_count, Texture2D zombiehand_texture) {
    float spawnDistance = 800.0f;
    
    for(int i = 0; i < count; i++) {
        // Initialize with invalid position first
        hands[i].x = -1;
        hands[i].y = -1;
        hands[i].width = 60;
        hands[i].height = 60;
        ...
        float potentialX = (i + 1) * spawnDistance;
        bool validPosition = false;
        
        // Check each platform
        for(int j = 0; j < total_ground_count; j++) {
            if (potentialX >= platforms[j].x && 
                potentialX <= platforms[j].x + platforms[j].width) {
                
                // Only set position if it's above a FLOOR
                if (platforms[j].type == FLOOR) {
                    hands[i].x = potentialX;
                    hands[i].y = platforms[j].y - hands[i].height - 30;
                    validPosition = true;
                    break;
                }
            }
        ...
        // If no valid position found, ensure it stays at -1
        if (!validPosition) {
            hands[i].x = -1;
            hands[i].y = -1;
            printf("Invalid position set for hand %d\n", i);
...
void UpdateZombieHands(ZombieHand hands[], int count, Player player) {
    for(int i = 0; i < count; i++) {
        ...
        if (hands[i].x == -1 || hands[i].y == -1) {
            continue;
        }

        // Rest of your update logic...
        if (!hands[i].isActive) {
            if (player.x > (hands[i].x - 800.0f)) {
...
void DrawZombieHands(ZombieHand hands[], int count) {
    for(int i = 0; i < count; i++) {
        ...
        printf("Hand %d: x=%d, y=%d, active=%d\n", 
               i, hands[i].x, hands[i].y, hands[i].isActive);
...
int main(void){
    ...
    InitZombieHands(zombie_hands, MAX_ZOMBIE_HANDS, platforms, total_ground_count, zombiehand_texture);
    ...
    while (!WindowShouldClose()){
        ...
        if(gameState == START_SCREEN){
            ...
            UpdateZombieHands(zombie_hands, MAX_ZOMBIE_HANDS, player);
            DrawPlayer(player);
