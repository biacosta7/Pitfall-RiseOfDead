...
int main(void){
    ...
    while (!WindowShouldClose()){
        ...
        if(gameState == START_SCREEN){
            ...
            //if (IsKeyPressed(KEY_ENTER) && strlen(nome_player) > 0) {
            //  add_winner(&head, nome_player, elapsedTime); // Salva o jogador na lista de vencedores
            //  gameState = END; // Transição para o estado final
            //}
            //ClearBackground(BLACK);
