    ...
    for(int i = 0; i < platformCount; i++) {
        ...
        if(CheckCollisionRecs(player_rec, platform_rec)) {
            if (platforms[i].type == FLOOR) {
                ...
                // Treat PLATFORM platforms differently
                printf("PLATFORM\n");
                return i;
            }
            return i;
...
void aplica_gravidade(Player *player, Enemy *enemy, Platform platforms[], int platform_count, float deltaTime) {
    ...
    if (current_platform != -1) {
        ...
        if (player->velocityY > 0) {  // Only if moving downward
            ...
            if (platforms[current_platform].type == FLOOR) {
                // Snap to FLOOR platform top
                player->y = platforms[current_platform].y - (player->height * 6);
                player->velocityY = 0;
                player->isJumping = false;
            } else if (platforms[current_platform].type == PLATFORM) {
                // Snap to PLATFORM top, but allow jumping
                player->y = platforms[current_platform].y - (player->height * 6);
                player->velocityY = 0;
            }
        }
...
int main(void){
    ...
    while (!WindowShouldClose()){
        ...
        if(gameState == START_SCREEN){
            ...
            //desenhar floor
            // At initialization, make sure these are defined

            // In your drawing code
            for(int i = 0; i < total_platform_count; i++) {
                // First determine the platform type
                if (platform1_streak >= platform1_count) {
                    platforms[i].type = PLATFORM;
                    platform1_streak = 0; // Reset streak
                } else {
                    platforms[i].type = FLOOR;
                    platform1_streak++;
                }

                // Set platform position and dimensions
                platforms[i].x = platform_x;
                platforms[i].y = screenHeight - floor_piece_height + whitespace;
                platforms[i].width = platform_width;
                platforms[i].height = platform_height;
                ...
                if (platform1_texture.id <= 0 || platform2_texture.id <= 0) {
                    printf("Warning: Textures not properly loaded!\n");
                }

                // Select and draw texture based on platform type
                Texture2D platform_texture_final;
                if (platforms[i].type == FLOOR) {
                    platform_texture_final = platform1_texture;
                    printf("Drawing FLOOR \n");
                } else {
                    platform_texture_final = platform2_texture;
                    printf("Drawing PLATFORM \n");
                }

                // Draw debug rectangle to see platform boundaries
                DrawRectangleLines(platforms[i].x, platforms[i].y, 
                                platforms[i].width, platforms[i].height, RED);

                // Draw the actual texture
                DrawTexture(platform_texture_final, platforms[i].x, platforms[i].y - whitespace, WHITE);
            }
