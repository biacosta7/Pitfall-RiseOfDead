...
int player_na_platforma(Player player, Platform platforms[], int platformCount) {
    for(int i = 0; i < platformCount; i++) {
        ...
        if(CheckCollisionRecs(player_rec, platform_rec)) {
            printf("colidiu com o chao\n");
            return i;
        }
    }
    return -1;
}
void aplica_gravidade(Player *player, Enemy *enemy, Platform platforms[], int platform_count) {
...
int main(void){
    ...
    while (!WindowShouldClose()){
        ...
        if(gameState == START_SCREEN){
            ...
            aplica_gravidade(&player, &enemy, platforms, platform_count);
